...
import { addUTMParametersToURLIfNecessary, copyLinkAddressLabel, MaxLengthForDisplayedURLs, openLinkExternallyLabel, } from './UIUtils.js';
  ...
  jslog=${VisualLogging.link().track({ click: true, keydown: 'Enter|Space' }).context(jsLogContext)}>${Platform.StringUtilities.trimMiddle(linkText, MaxLengthForDisplayedURLs)}</x-link>`;
    ...
    attributeChangedCallback(attr, oldValue, newValue) {
        ...
        if (attr === 'href') {
            ...
            let url = null;
            try {
                url = new URL(addUTMParametersToURLIfNecessary(newValue));
                href = url.toString();
            }
            catch {
            }
